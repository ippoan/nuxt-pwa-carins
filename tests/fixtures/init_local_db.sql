-- rust-alc-api ローカルテスト用 DB 初期化 (carins integration test)
-- 本番 Supabase と同等のスキーマ・ロール構成を再現する。
-- alc-app の web/tests/fixtures/init_local_db.sql と同じ役割。
...
-- (migration の非修飾テーブル名が alc_api スキーマに作成されるように)
